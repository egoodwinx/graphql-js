...
describe('extendSchema', () => {
  ...
  describe('field merging', () => {
    ...
    it('merges field arguments additively', () => {
      ...
      const searchFieldConfig = searchField.toConfig();

      expect(Object.keys(searchFieldConfig.args)).to.have.members([
        'query',
        'limit',
      ]);
      expect(searchFieldConfig.args.query.type.toString()).to.equal('String');
      expect(searchFieldConfig.args.limit.type.toString()).to.equal('Int');
    ...
    it('merges multiple extensions on the same field', () => {
      ...
      const profileFieldConfig = profileField.toConfig();
      expect(Object.keys(profileFieldConfig.args)).to.have.members([
        'format',
        'detailed',
        'includePrivate',
      ]);
    ...
    it('preserves original field properties when not overridden', () => {
      ...
      const fieldConfig = field.toConfig();
      expect(Object.keys(fieldConfig.args)).to.have.members(['arg', 'newArg']);
      expect(fieldConfig.args.arg.type.toString()).to.equal('String');
      expect(fieldConfig.args.newArg.type.toString()).to.equal('Int');
